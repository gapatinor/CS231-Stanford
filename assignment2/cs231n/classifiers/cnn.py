...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        Fs = filter_size
        Nf = num_filters
        w1 = weight_scale * np.random.randn(Nf, C, Fs, Fs)
        b1 = np.zeros(Nf)
        w2 = weight_scale * np.random.randn(Nf*H_conv*W_conv, hidden_dim)
    ...
    def loss(self, X, y=None):
        ...
        dl3, dW3, db3 = affine_backward(dloss, l3_cache)
        dl2, dW2, db2 = affine_relu_backward(dl3, l2_cache)
        dX, dW1, db1 = conv_relu_pool_backward(dl2, l1_cache)
